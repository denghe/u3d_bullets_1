...
public class SpaceContainer {
    ...
    /// <summary>
    /// 圆形扩散遍历. handler 返回 true 表示停止遍历
    /// </summary>
    public void ForeachByRange(SpaceRingDiffuseData d, float x, float y, float maxDistance, Func<SpaceItem, bool> handler) {
        ...
        for (int i = 1; i < lens.Count; i++) {
            ...
            if (lens[i].radius > searchRange) return;
        }
    }
