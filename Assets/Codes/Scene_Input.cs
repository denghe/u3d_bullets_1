...
partial class Scene {
    ...
    /// <summary>
    /// 归一化之后的移动方向( 读前先判断 playerMoving )
    /// </summary>
    internal Vector2 playerMoveValue;

    /// <summary>
    /// 最后移动朝向角度( 根据 playerMoveValue 计算所得 )
    /// </summary>
    internal float playerMoveRadians;
    ...
    internal void HandlePlayerInput() {
        if (playerUsingKeyboard) {      // 键盘需要每帧判断, 合并方向, 计算最终矢量
        ...
        if (playerMoveValue.x != 0 || playerMoveValue.y != 0) {
            playerMoveRadians = Mathf.Atan2(playerMoveValue.y, playerMoveValue.x);
        }
    }
}
